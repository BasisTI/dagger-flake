{
  stdenv,
  fetchurl,
  lib,
}:

let
  pname = "dagger";
  version = "0.20.0";
  platformInfo = {
  "aarch64-darwin" = {
    arch = "darwin_arm64";
    sha256 = "sha256-KokaCBDmJxq2skQj2kuulTz1GW8DIBsupg7aLMrD7uI=";
  };
  "aarch64-linux" = {
    arch = "linux_arm64";
    sha256 = "sha256-9sE5CgcK3Cnm+Oh4otwr9AiFpSibwcCtq3XC/YRRMAU=";
  };
  "x86_64-darwin" = {
    arch = "darwin_amd64";
    sha256 = "sha256-JWJRHF5Shc8ikXtBYJwr5mRN0ItTmzcIGLKiaU213DI=";
  };
  "x86_64-linux" = {
    arch = "linux_amd64";
    sha256 = "sha256-sywgcj+fUSGn3+5QaC4AMrJbmX/rtIEUgc/OIHSoiwI=";
  };
};













  platform = lib.getAttr stdenv.hostPlatform.system platformInfo;
in
stdenv.mkDerivation {
  inherit pname version;

  src = fetchurl {
    url = "https://github.com/dagger/dagger/releases/download/v${version}/${pname}_v${version}_${platform.arch}.tar.gz";
    sha256 = platform.sha256;
  };

  phases = [
    "unpackPhase"
    "installPhase"
  ];
  unpackPhase = "tar -xzf $src";
  installPhase = "install -Dm755 dagger $out/bin/dagger";

  meta = with lib; {
    description = "Dagger CLI – composable CI/CD runtime";
    homepage = "https://dagger.io";
    license = licenses.asl20;
    maintainers = with maintainers; [ cedric-lamalle ];
    platforms = platforms.darwin ++ platforms.linux;
  };
}
